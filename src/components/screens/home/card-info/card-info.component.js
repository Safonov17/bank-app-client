...
export class CardInfo extends ChildComponent {
	...
	#toggleCvc(cvcCardElement) {
		const text = cvcCardElement.text()
		text === CODE
			? cvcCardElement.text(this.card.cvc)
			: cvcCardElement.text(CODE)
	}
	...
	fillElements() {
		...
		const cvcCardElement = $R(this.element).find('#card-cvc')
		cvcCardElement.text(CODE).css('width', '44px')

		$R(this.element)
			.find('#toggle-cvc')
			.click(this.#toggleCvc.bind(this, cvcCardElement))
